...
def main():
    ...
    starting_date = get_positive_int("Enter the starting date for commits (YYYYMMDD)", 20260726)
    ending_date = get_positive_int("Enter the ending date for commits (YYYYMMDD)", 20260730)

    days_difference = (datetime.strptime(str(ending_date), "%Y%m%d") - datetime.strptime(str(starting_date), "%Y%m%d")).days

    print(f"\nMaking {days_difference + 1} commits in repo: {repo_path}\nModifying file: {filename}\n")

    for i in range(days_difference + 1):
        message = input(f"Enter commit message for day {i+1}: ")
        commit_date = datetime(starting_date // 10000, (starting_date // 100) % 100, starting_date % 100) + timedelta(days=i)
        print(f"[{i+1}/{days_difference + 1}] Committing at {commit_date.strftime('%Y-%m-%d %H:%M:%S')}")
