...
class EmailFormClass(FlaskForm):
    e_recipients = StringField(
        label="Recipients",
        description="Enter your recipients with coma(,) separated",
        validators=[DataRequired(), Email()],
    )
    e_subject = StringField(
        "Subject",
        description="Enter the subject of the email",
        validators=[Length(min=3, max=100)],
    )
    e_message = TextAreaField(
        "Message", description="Write the message", validators=[Length(min=5)]
    )
    e_attachment = FileField(
        "Attachment",
        description="Add a attachment",
        validators=[
            FileRequired(),
            FileAllowed(
                ["jpg", "jpeg", "png"], "Only images and pdf files are allowed"
            ),
        ],
    )
